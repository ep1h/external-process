...
typedef struct EPCFunctionCaller
{
                            ...
                            EPCVirtualProtect* out_old_protect)
{
    ...
    DWORD old_protect = 0;
    new_protect = convert_ep_protect_to_win_protect(protect);
    if (NtProtectVirtualMemory(process->handle, (PVOID*)&address, (PULONG)&size,
...
bool epc_function_caller_destroy(EPCFunctionCaller* caller)
{
    if (!caller)
    {
        return false;
...
bool epc_function_caller_send_args(const EPCFunctionCaller* caller,
                                   size_t argc, ...)
{
...
bool epc_function_caller_call(const EPCFunctionCaller* caller,
                              uintptr_t* out_result, bool wait_for_return)
{
    ...
    DWORD result =
        WaitForSingleObject(thread_handle, wait_for_return ? INFINITE : 0);
